...
public class ReservationDAO {
	...
	private final String SELECTALL = "SELECT R.RESERVATION_NUM, R.RESERVATION_REGISTRATION_DATE, R.RESERVATION_STATUS, COALESCE(PR.PRODUCT_NUM,0) AS PRODUCT_NUM, COALESCE(PR.PRODUCT_NAME,'존재하지 않는 상품입니다.') AS PRODUCT_NAME, P.PAYMENT_PRICE\r\n"
			...
			+ "WHERE P.PAYMENT_MEMBER_ID = ? ORDER BY R.RESERVATION_NUM DESC";

	// 내 예약 상세보기
	private final String SELECTONE = "SELECT R.RESERVATION_NUM, R.RESERVATION_REGISTRATION_DATE, R.RESERVATION_STATUS, COALESCE(PR.PRODUCT_NUM,0) AS PRODUCT_NUM, COALESCE(PR.PRODUCT_NAME,'존재하지 않는 상품입니다.') AS PRODUCT_NAME, P.PAYMENT_PRICE, P.PAYMENT_REGISTRATION_DATE,\r\n"
