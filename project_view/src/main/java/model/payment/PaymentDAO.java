...
public class PaymentDAO {
	...
	// 결제 생성
	private final String INSERT = "INSERT INTO PAYMENT (PAYMENT_NUM,PAYMENT_MEMBER_ID,PAYMENT_PRODUCT_NUM,PAYMENT_ORDER_NUM,MERCHANT_UID,PAYMENT_PRICE,PAYMENT_STATUS,PAYMENT_METHOD)\r\n"
			+ "VALUES(COALESCE((SELECT MAX(PAYMENT_NUM) FROM PAYMENT),0)+1,?,?,?,?,?,?,?)";
	// 결제 변경 (삭제 없으므로 결제 상태 변경)
	private final String UPDATE = "UPDATE PAYMENT SET PAYMENT_STATUS =? WHERE MERCHANT_UID=?";
	// 결제 내역 반환
	private final String SELECTONE = "SELECT PAYMENT_NUM,PAYMENT_MEMBER_ID,PAYMENT_PRODUCT_NUM,PAYMENT_ORDER_NUM,MERCHANT_UID,PAYMENT_REGISTRATION_DATE,PAYMENT_PRICE,PAYMENT_STATUS,PAYMENT_METHOD\r\n"
			+ "FROM PAYMENT\r\n"
			+ "WHERE MERCHANT_UID=?";
	...
	public boolean update(PaymentDTO paymentDTO){	// 변경
		...
		try {
			...
			pstmt.setString(2, paymentDTO.getMerchant_uid()); // 컨트롤러에서 사용하는 관리 번호 (UUID+시간 조합)
	...
	public PaymentDTO selectOne(PaymentDTO paymentDTO){// 한명 출력
		...
		try {
			...
			// 파라미터에 찾을 데이터를 넣어준다.
			pstmt.setString(1, paymentDTO.getMerchant_uid()); // 컨트롤러에서 사용하는 번호
