...
public class WriteReviewAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		// 리뷰 작성
		System.out.println("WriteReviewAction 시작");
		...
		if(flag) {
			System.out.println("WriteReviewAction 로그 : 리뷰 작성 성공");
			forward.setRedirect(true);
			forward.setPath("productDetail.do?productNum="+review_product_num);
		}
		else {
			System.out.println("WriteReviewAction 로그 : 리뷰 작성 실패");
			request.setAttribute("msg", "리뷰 작성에 실패했습니다. 다시 시도해주세요.");
			request.setAttribute("path", "productDetail.do?product_num="+review_product_num);
			forward.setRedirect(false);
			forward.setPath("info.jsp");
		}
		System.out.println("WriteReviewAction 끝");
		return forward;
	}
