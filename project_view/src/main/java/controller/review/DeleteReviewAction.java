...
public class DeleteReviewAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		int review_num = Integer.parseInt(request.getParameter("reivew_num"));
		...
		if(flag) {
			...
			forward.setPath("productDetail.do?productNum="+product_num);
		}
		else {
			System.out.println("DeleteReviewAction 로그 : 리뷰 삭제 실패");
			request.setAttribute("msg", "리뷰 삭제에 실패했습니다. 다시 시도해주세요.");
			request.setAttribute("path", "productDetail.do?productNum="+product_num);
			forward.setRedirect(false);
			forward.setPath("info.jsp");
		}
