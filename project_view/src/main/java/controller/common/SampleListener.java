...
public class SampleListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		// 크롤링 객체 생성
		CrawlingSelenium crawling = new CrawlingSelenium(); // 크롤링 객체
		ProductDAO productDAO = new ProductDAO(); // 상품 객체
		BoardDAO boardDAO = new BoardDAO();	// 게시판 객체
		FileDAO imageFileDAO = new FileDAO();	// 이미지 객체

		// 상품 데이터 처리
		processProducts(productDAO, imageFileDAO);

		// 게시판 데이터 처리
		processBoards(boardDAO, imageFileDAO);
	...
	private void insertProducts(ProductDAO productDAO, FileDAO fileDAO, ArrayList<ProductDTO> products, String condition) {
		...
		for (ProductDTO data : products) {// 상품 리스트 만큼 반복
			...
			if (isInserted) { // insert가 true가 되면
				...
				System.out.println("82 로그 product="+product);
				...
				if(data.getUrl() != null && !data.getUrl().isEmpty()) {
					for (String imageUrl : data.getUrl()) { 
						...
						imageFileDTO.setFile_condition("FILE_PRODUCT_CRAWLING_INSERT"); // 파일 상태 설정
						imageFileDTO.setFile_original_name(data.getProduct_name()); // 원래 파일명
						System.out.println("producName:"+data.getProduct_name());
						imageFileDTO.setFile_product_num(product.getProduct_num()); // 해당 제품의 번호
						System.out.println("productNum:"+product.getProduct_num());
						imageFileDTO.setFile_dir(imageUrl); // 이미지 URL 설정 (공백 제거)

						// 이미지 DB에 삽입
						fileDAO.insert(imageFileDTO);
						System.out.println("87 로그: 이미지 삽입 완료 - 이미지 URL: " + imageUrl);
					}
				} 
			}
			else {
				System.out.println("제품 삽입 실패 - 제품명: [" + data.getProduct_name()+"]");
			}
		}
	}
	...
	private void processBoards(BoardDAO boardDAO, FileDAO fileDAO) {
		System.out.println("92 로그: 게시판 데이터 처리 시작");
		...
		if (boardDAO.selectOne(boardDTO) == null) {
			...
			insertBoards(boardDAO, fileDAO, boards);
			System.out.println("95 로그: 게시판 데이터 처리 완료");
		}
		else {
	...
	private void insertBoards(BoardDAO boardDAO, FileDAO fileDAO, ArrayList<BoardDTO> boards) {
		for (BoardDTO data : boards) {
			...
			data.setBoard_condition("BOARD_CRAWLING_INSERT"); // 게시판 상태 설정
			...
			if (isInserted) {
				...
				if(data.getUrl()!=null&&!data.getUrl().isEmpty()) {
					for (String imageUrl : data.getUrl()) {
						...
						// 이미지 DB에 삽입
						fileDAO.insert(imageFileDTO);
						System.out.println("181 로그: 이미지 삽입 완료 - 이미지 URL: " + imageUrl);

					}
				}
			}else {
				System.out.println("제품 삽입 실패 - 제품명: [" + data.getBoard_title()+"]");
			}
		} 
	}
