...
WHERE TABLE_NAME = 'IMAGEFILE';

/*
-- 컬럼 추가
ALTER TABLE IMAGEFILE ADD FILE_ORIGINAL_NAME VARCHAR(100) DEFAULT '' NOT NULL;
-- DEFAULT 값 삭제
ALTER TABLE IMAGEFILE MODIFY FILE_ORIGINAL_NAME DEFAULT NULL;


SELECT COLUMN_NAME, DATA_TYPE, DATA_LENGTH, DATA_PRECISION, DATA_SCALE, NULLABLE, DEFAULT_LENGTH, DATA_DEFAULT
FROM USER_TAB_COLUMNS
WHERE TABLE_NAME = 'IMAGEFILE'

SELECT CONSTRAINT_NAME, CONSTRAINT_TYPE FROM USER_CONSTRAINTS  WHERE TABLE_NAME = 'IMAGEFILE';
*/
...
VALUES (NVL((SELECT MAX(FILE_NUM) FROM IMAGEFILE),0)+1,'sample_image1.png','sample_image1','.png','/images/sample_image1.png',2,'상품');
INSERT INTO IMAGEFILE
VALUES (NVL((SELECT MAX(FILE_NUM) FROM IMAGEFILE),0)+1,'sample_image2.png','sample_image2','.png','/images/sample_image1.png',4,'상품');
INSERT INTO IMAGEFILE (FILE_NUM,FILE_ORIGINAL_NAME,FILE_NAME,FILE_EXTENSION,FILE_DIR,ITEM_NUM,FILE_CONTENT_TYPE) 
VALUES (NVL((SELECT MAX(FILE_NUM) FROM IMAGEFILE),0)+1,'PHOTO.jpg','PHOTO','jpg','images/.jpg',2,'상품');
...
FROM IMAGEFILE WHERE ITEM_NUM=? AND FILE_CONTENT_TYPE=? ORDER BY FILE_NUM;
