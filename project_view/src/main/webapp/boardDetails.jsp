							...
							<c:forEach var="file" items="${fileList}">
								<div class="ts-item">
									<div class="bd-pic">
										<!-- 사용자 입력 이미지 -->
										<img
											src="${file.file_dir}${file.file_images}${file.file_extention}"
						...
						<div class="tag-share">
							<div class="tags">
								<div class="button-container">
									...
									<input id="like_member" value="${like_member}"
										style="visibility: hidden;">
									<!-- 로그인한 사용자와 작성자가 같은 경우 -->
									<c:if test="${member_id == board.board_writer_id}">
										<!-- 글삭제 버튼 -->
										<button id="editButton" class="likeButton">❌삭제</button>
										<!-- 글수정 버튼 -->
										<button id="reportButton" class="likeButton">🛠 수정</button>
									</c:if>
								</div>
							</div>
						</div>
						<div class="comment-option">
							...
							<c:forEach var="reply" items="${replyList}">
								<div class="single-comment-item">
									<div class="sc-author">
										<!-- 댓글 작성자 프로필 이미지 -->
										<img src="${reply.reply_member_profile}" alt="댓글 작성자 프로필 이미지">
									</div>
									<div class="sc-text">
										<!-- 댓글 작성일자 -->
										...
										<!-- 댓글 삭제 (비동기X)-->
										<c:if test="${member_id == board.board_writer_id}">
											<a href="deleteReply.do?boardNum=${board.board_num}"
												class="comment-btn">댓글삭제</a>
										</c:if>
									</div>
								</div>
							</c:forEach>
