...
let reserevationDate = $('#reservation_date').val();
console.log("예약일 : "+reserevationDate);
		...
		if (rsp.success) {
		  ...
		  jQuery.ajax({
		      ...
		      if(rsp.paid_amount == data){
		          ...
		          alert("결제 및 결제검증완료");
		          location.href='makeReservation.do?merchant_uid='+merchant_uid+'&&reservation_registration_date'+reserevationDate;
		      } else {
		         alert("검증 실패");
				 // 결제 취소
