$(document).ready(function() {
	var flag = '${like_member}' === 'true';; // 회원이 좋아요를 눌렀는지 여부를 나타내는 상태값
	...
	$('#likeBtn').on('click', function() {
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.likeCnt) { // 성공 플래그에 따라 조건 변경
					likeCount.text(response.likeCnt);
					// 좋아요를 눌렀으면, 하늘색/ 안눌렀으면 빨간색 버튼으로 변경
					$('#likeBtn').css('backgroundColor', flag ? 'red' : '#87CEEB');
					console.log("성공");
				} else {
					console.log("실패");
